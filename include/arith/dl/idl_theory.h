...
namespace semitone
{
  ...
  public:
    ...
    SEMITONE_EXPORT inline I lb(const var &v) const noexcept { return -_dists[v][0]; }
    SEMITONE_EXPORT inline I ub(const var &v) const noexcept { return _dists[0][v]; }
    SEMITONE_EXPORT inline std::pair<I, I> bounds(const var &v) const noexcept { return std::make_pair(-_dists[v][0], _dists[0][v]); }
    SEMITONE_EXPORT inline std::pair<I, I> distance(const var &from, const var &to) const noexcept { return std::make_pair(-_dists[to][from], _dists[from][to]); }
