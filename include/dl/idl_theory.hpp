...
namespace semitone
{
  ...
  public:
     ...
     */
    [[nodiscard]] utils::lit new_distance(utils::var from, utils::var to, INT_TYPE dist) noexcept;
    /**
     * @brief Creates a new distance between the given variables and returns the corresponding literal.
     *
     ...
     * @return utils::lit the literal corresponding to the distance.
     */
    [[nodiscard]] utils::lit new_distance(utils::var from, utils::var to, INT_TYPE min, INT_TYPE max) noexcept;
     ...
     * @return INT_TYPE the lower bound of the variable.
     */
    [[nodiscard]] inline INT_TYPE lb(utils::var v) const noexcept { return -dists[v][0]; }
    /**
     * @brief Returns the upper bound of the given variable.
     *
     * @param v the variable to get the upper bound of.
     * @return INT_TYPE the upper bound of the variable.
     */
    [[nodiscard]] inline INT_TYPE ub(utils::var v) const noexcept { return dists[0][v]; }
    /**
     ...
     */
    [[nodiscard]] inline std::pair<INT_TYPE, INT_TYPE> bounds(utils::var v) const noexcept { return {-dists[v][0], dists[0][v]}; }
    /**
     ...
     */
    [[nodiscard]] inline std::pair<INT_TYPE, INT_TYPE> distance(utils::var from, utils::var to) const noexcept { return {-dists[to][from], dists[from][to]}; }
  ...
  private:
    ...
    void propagate(utils::var from, utils::var to, INT_TYPE dist) noexcept;
    [[nodiscard]] bool check() noexcept override { return true; }
    void push() noexcept override;
    void pop() noexcept override;

    void set_dist(utils::var from, utils::var to, INT_TYPE dist) noexcept;
  ...
  private:
    ...
    struct layer
    {
      std::map<std::pair<utils::var, utils::var>, INT_TYPE> old_dists;                                                               // the updated distances..
      std::map<std::pair<utils::var, utils::var>, utils::var> old_preds;                                                             // the updated predecessors..
      std::map<std::pair<utils::var, utils::var>, std::optional<std::reference_wrapper<distance_constraint<INT_TYPE>>>> old_constrs; // the updated constraints..
    };
  ...
  private:
    ...
    std::unordered_map<utils::var, std::unique_ptr<distance_constraint<INT_TYPE>>> var_dists;                                     // the constraints controlled by a propositional variable (for propagation purposes)..
    std::map<std::pair<utils::var, utils::var>, std::vector<std::reference_wrapper<distance_constraint<INT_TYPE>>>> dist_constrs; // the constraints between two temporal points (for propagation purposes)..
    std::map<std::pair<utils::var, utils::var>, std::reference_wrapper<distance_constraint<INT_TYPE>>> dist_constr;               // the currently enforced constraints..
    std::vector<layer> layers;                                                                                                    // we store the updates..
#ifdef BUILD_LISTENERS
  private:
    std::unordered_map<utils::var, std::set<idl_value_listener *>> listening; // for each variable, the listeners listening to it..
