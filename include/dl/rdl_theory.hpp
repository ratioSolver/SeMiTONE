...
namespace semitone
{
  ...
  public:
     ...
     */
    [[nodiscard]] utils::lit new_distance(utils::var from, utils::var to, const utils::inf_rational &dist) noexcept;
    /**
     * @brief Creates a new distance between the given variables and returns the corresponding literal.
     *
     ...
     * @return utils::lit the literal corresponding to the distance.
     */
    [[nodiscard]] utils::lit new_distance(utils::var from, utils::var to, const utils::inf_rational &min, const utils::inf_rational &max) noexcept;
     ...
     */
    [[nodiscard]] inline utils::inf_rational lb(utils::var v) const noexcept { return -dists[v][0]; }
    /**
     ...
     */
    [[nodiscard]] inline utils::inf_rational ub(utils::var v) const noexcept { return dists[0][v]; }
    /**
     ...
     */
    [[nodiscard]] inline std::pair<utils::inf_rational, utils::inf_rational> bounds(utils::var v) const noexcept { return {-dists[v][0], dists[0][v]}; }
    /**
     ...
     */
    [[nodiscard]] inline std::pair<utils::inf_rational, utils::inf_rational> distance(utils::var from, utils::var to) const noexcept { return {-dists[to][from], dists[from][to]}; }
  ...
  private:
    ...
    struct layer
    {
      ...
      std::map<std::pair<utils::var, utils::var>, std::optional<std::reference_wrapper<distance_constraint<utils::inf_rational>>>> old_constrs; // the updated constraints..
    };
  ...
  private:
    ...
    std::unordered_map<utils::var, std::unique_ptr<distance_constraint<utils::inf_rational>>> var_dists;                                     // the constraints controlled by a propositional variable (for propagation purposes)..
    std::map<std::pair<utils::var, utils::var>, std::vector<std::reference_wrapper<distance_constraint<utils::inf_rational>>>> dist_constrs; // the constraints between two temporal points (for propagation purposes)..
    std::map<std::pair<utils::var, utils::var>, std::reference_wrapper<distance_constraint<utils::inf_rational>>> dist_constr;               // the currently enforced constraints..
    std::vector<layer> layers;                                                                                                               // we store the updates..
#ifdef BUILD_LISTENERS
  private:
    std::unordered_map<utils::var, std::set<rdl_value_listener *>> listening; // for each variable, the listeners listening to it..
