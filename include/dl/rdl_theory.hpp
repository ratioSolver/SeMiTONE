...
namespace semitone
{
  ...
  public:
     ...
     */
    [[nodiscard]] inline std::pair<utils::inf_rational, utils::inf_rational> bounds(VARIABLE_TYPE v) const noexcept { return std::make_pair(-dists[v][0], dists[0][v]); }
    /**
     ...
     */
    [[nodiscard]] inline std::pair<utils::inf_rational, utils::inf_rational> distance(VARIABLE_TYPE from, VARIABLE_TYPE to) const noexcept { return std::make_pair(-dists[to][from], dists[from][to]); }
