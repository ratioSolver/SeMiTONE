...
namespace smt
{
  ...
  private:
    ...
    std::map<const utils::var, std::vector<std::unique_ptr<la_assertion>>> v_asrts; // the assertions (literal to assertions) used for enforcing (negating) assertions..
    std::map<const utils::var, std::unique_ptr<la_eq>> tableau;                     // the tableau..
    std::vector<std::vector<std::reference_wrapper<la_assertion>>> a_watches;       // for each variable `v`, a list of assertions watching `v`..
