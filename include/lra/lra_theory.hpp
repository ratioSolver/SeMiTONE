...
namespace semitone
{
  ...
  public:
    lra_assertion(const utils::lit b, const std::size_t x, const op o, const utils::inf_rational &v) noexcept : b(b), x(x), o(o), v(v) {}
  ...
  private:
    ...
    const std::size_t x;       // the numeric variable..
  ...
  public:
    lra_eq(const std::size_t x, const utils::lin &&l) noexcept : x(x), l(std::move(l)) {}
  ...
  public:
     ...
     */
    [[nodiscard]] std::size_t new_var(const utils::inf_rational &lb = utils::inf_rational(utils::rational::negative_infinite), const utils::inf_rational &ub = utils::inf_rational(utils::rational::positive_infinite)) noexcept;
     ...
     * @return std::size_t the new variable.
     */
    [[nodiscard]] std::size_t new_var(const utils::lin &&l) noexcept;
     ...
     */
    [[nodiscard]] utils::lit new_leq(const std::size_t x, const utils::inf_rational &v) noexcept;
     ...
     * @return A utils::lit object representing the inequality x >= v.
     */
    [[nodiscard]] utils::lit new_geq(const std::size_t x, const utils::inf_rational &v) noexcept;
     ...
     */
    [[nodiscard]] inline utils::inf_rational lb(const std::size_t v) const noexcept { return c_bounds[lb_index(v)].value; }
    /**
     ...
     */
    [[nodiscard]] inline utils::inf_rational ub(const std::size_t v) const noexcept { return c_bounds[ub_index(v)].value; }
    /**
     ...
     */
    [[nodiscard]] inline utils::inf_rational value(const std::size_t v) const noexcept { return vals[v]; }
     ...
     */
    [[nodiscard]] bool set_lb(const std::size_t x_i, const utils::inf_rational &val, const std::vector<utils::lit> &r = {}) noexcept;
    /**
     ...
     * @return bool whether the propagation was successful.
     */
    [[nodiscard]] bool set_ub(const std::size_t x_i, const utils::inf_rational &val, const std::vector<utils::lit> &r = {}) noexcept;
  ...
  private:
    ...
    [[nodiscard]] inline static size_t lb_index(const std::size_t v) noexcept { return v << 1; }       // the index of the lower bound of the `v` variable..
    [[nodiscard]] inline static size_t ub_index(const std::size_t v) noexcept { return (v << 1) ^ 1; } // the index of the upper bound of the `v` variable..

    [[nodiscard]] bool is_basic(const std::size_t v) const noexcept { return tableau.count(v); }

    [[nodiscard]] bool assert_lower(const std::size_t x_i, const utils::inf_rational &val, const std::vector<utils::lit> &r) noexcept;
    [[nodiscard]] bool assert_upper(const std::size_t x_i, const utils::inf_rational &val, const std::vector<utils::lit> &r) noexcept;

    void update(const std::size_t x_i, const utils::inf_rational &v) noexcept;
    void pivot_and_update(const std::size_t x_i, const std::size_t x_j, const utils::inf_rational &v) noexcept;
    void pivot(const std::size_t x_i, const std::size_t x_j) noexcept;
    void new_row(const std::size_t x_i, const utils::lin &&xpr) noexcept;
  ...
  private:
    ...
    std::map<const std::size_t, std::unique_ptr<lra_assertion>> v_asrts;     // the assertions (literal to assertions) used for enforcing (negating) assertions..
    std::map<const std::size_t, std::unique_ptr<lra_eq>> tableau;            // the tableau..
    std::vector<std::vector<std::reference_wrapper<lra_assertion>>> a_watches; // for each variable `v`, a list of assertions watching `v`..
    std::vector<std::set<std::size_t>> t_watches;                            // for each variable `v`, a list of tableau rows watching `v`..
    std::unordered_map<std::string, std::size_t> exprs;                      // the expressions (string to numeric variable) for which already exist slack variables..
...
#ifdef BUILD_LISTENERS
  private:
    std::unordered_map<std::size_t, std::set<lra_value_listener *>> listening; // for each variable, the listeners listening to it..
